...
public class BattleCharacter : CharacterProperty
{
    ...
    IEnumerator OnDmg(float dmg,int a,GameObject myTarget) //?¡À????????
    {
        if (myTarget.GetComponent<BattleCharacter>().State == STATE.Die) yield break;
        GameObject hudText = Instantiate(hudDmgText, Canvas.transform); // ?¡À???????? ????
        ...
        hudText.transform.position = pos2; // ?¡À?????????? po2?????
