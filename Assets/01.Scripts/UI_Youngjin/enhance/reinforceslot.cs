...
public class reinforceslot : MonoBehaviour, IDropHandler        //마우스 우클릭하면 돈 100원씩 들어옵니다...-영진
{
    ...
    public void UpdateNumberUI(string name, int level, int index)
    {
        ...
        Amount[index].text = $"{DataController.instance.gameData.itemCount[i]} " + "/" + $" {level}";        //가지고 있는 개수와 몇개 필요한지 표시
    ...
    public void CheckIngredients(Transform thisitem)
    {
        ...
        {
            case "금반지":
                if (DataController.instance.gameData.itemList.Contains("다이아몬드") && DataController.instance.gameData.itemList.Contains("금")&&DataController.instance.gameData.gold>=cost)
                {
                ...
                break;
            case "장갑":
                if (DataController.instance.gameData.itemList.Contains("눈물") && DataController.instance.gameData.itemList.Contains("스크롤") && DataController.instance.gameData.gold >= cost)
                {
                ...
                break;
            case "목걸이":
                if (DataController.instance.gameData.itemList.Contains("별") && DataController.instance.gameData.itemList.Contains("철") && DataController.instance.gameData.gold >= cost)
                {
